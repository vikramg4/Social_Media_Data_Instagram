...
-- 1. Are there any tables with duplicate or missing null values? If so, how would you handle them?
   ...
   -- Checking for duplicate values in all the table
   ...
   -- Checking for null values for crucial columns in each table
   ...
   -- There are no duplicate and null values present in the data tables. 
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
-- 2. What is the distribution of user activity levels (e.g., number of posts, likes, comments) across the user base?
	  
   -- Distribution of the number of posts (photos) by each user
...
-- 3. Calculate the average number of tags per post (photo_tags and photos tables).
      ...
      WITH tags_per_post AS 
      (SELECT photo_id, COUNT(tag_id) AS tags
      FROM photo_tags
      GROUP BY photo_id)
